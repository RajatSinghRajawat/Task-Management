...
const API = `${getApiBaseUrl()}/api/materials`;
                          ...
                          {currentMaterial.type === 'Link' ? (
                             ...
                             <div className="space-y-3">
                                {currentMaterial.files?.map((f, i) => (
                                   <a key={i} href={`${getApiBaseUrl()}/${f}`} target="_blank" rel="noreferrer" className="w-full py-5 bg-white border border-slate-100 rounded-2xl flex items-center justify-center gap-3 font-bold text-[10px] uppercase tracking-wide text-slate-600 hover:bg-slate-900 hover:text-white transition-all group">
