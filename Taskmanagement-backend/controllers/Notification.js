...
const sendNotificationBroadcast = async (req, res) => {
    try {
        const { target, title, message } = req.body; // target: 'Students' | 'Teachers' | 'All'
        
        if (!title || !message || !target) {
            return res.status(400).json({ success: false, message: "Required fields missing (target, title, message)" });
        }
        ...
        if (target === 'Students' || target === 'All') {
            ...
            students.forEach(s => {
                recipients.push({ id: s._id, model: 'StudentAuth' });
            });
        }
        ...
        if (target === 'Teachers' || target === 'All') {
            ...
            teachers.forEach(t => {
                recipients.push({ id: t._id, model: 'User' });
            });
        }

        if (recipients.length === 0) {
            return res.status(400).json({ success: false, message: "No recipients found for target group" });
        }

        // Create notification documents for each recipient separately
        ...
        res.status(201).json({ 
            success: true, 
            message: `Announcement broadcast successfully to ${recipients.length} users.`,
