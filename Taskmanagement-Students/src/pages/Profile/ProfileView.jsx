...
const API_BASE = `${getApiBaseUrl()}/api/students`;
...
const ProfileView = () => {
  ...
  useEffect(() => {
     const fetchLatest = async () => {
        try {
           ...
           const activityRes = await axios.get(`${baseUrl}/api/students/${res.data._id}/activity`, {
              headers: { Authorization: `Bearer ${token}` }
           });
  ...
  const profileImageUrl = student.profileImage 
     ? `${baseUrl}/${student.profileImage.replace(/\\/g, '/')}`
                  ...
                  {student.resume ? (
                     <div className="p-6 bg-emerald-50/50 rounded-3xl border border-emerald-100 flex items-center justify-between group/res transition-all hover:bg-emerald-50">
                        ...
                        <div className="flex items-center gap-2">
                           <a 
                              href={`${getApiBaseUrl()}/${student.resume}`} 
                              target="_blank" 
